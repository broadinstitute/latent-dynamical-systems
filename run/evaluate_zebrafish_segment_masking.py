...
from utils.data_utils import load_data, preprocess_data, preprocess_coords_zero_masked, set_timepoints, set_num_steps
...
LOSS_WEIGHTS = {k: 1.0 for k in ALL_TIMEPOINTS}
if FIT_GEX0 == False:
...
inverse_mask_segments_mode = args.inverse_mask_mode
mask_segments_tp = args.mask_tp
tp, tp_masked,gex = preprocess_data(x, args, filter=FILTER, preprocess_gex=PREPROCESS_GEX, mask_segments=True, mask_segments_tp=mask_segments_tp,mask_segments_mode=inverse_mask_segments_mode)

size, yshifts, xshifts = set_scale(SCALE)

zero_pos, pos, nonzero_ims = preprocess_coords_zero_masked(tp, size=size, yshifts=yshifts, xshifts=xshifts, mask=True, tp_masked=tp_masked)
...
for rep in range(args.num_reps):
    ...
    ## TODO: this fails with only 2 timepoints 
    try:
            ...
            with torch.no_grad():
                for sel_ind, sel in enumerate(ALL_TIMEPOINTS):
                    ...
                    if RANDOM_BATCHING:
                        for _ in range(100):
                            samp = np.random.choice(idxs[sel], BATCH_SIZE)
                            ge_loss_reg, ge_diff_im = compute_batched_ge_loss_eval(pos[sel], t2_masked, dec_eval, ge, RECON_DIM, samp, ge_diff_im)
                            ge_diff_im += ge_diff_im
                            ge_loss_reg_batches.append(ge_loss_reg)

                            zer_samp = np.random.choice(zer_inds, int(ratio*BATCH_SIZE))
                            zero_loss_reg = compute_batched_zer_loss_eval(zero_pos[sel], t2_masked, dec_eval, RECON_DIM, zer_samp)
                        ...
                        for i in range(0, zer.shape[0], BATCH_SIZE):
                            ...
                            zero_loss_reg = zer_loss(t2_masked[zer_i, zer_j], RECON_DIM, dec_eval).detach().cpu().numpy()
                            zero_loss_reg_batches.append(zero_loss_reg)
                    
                    outline_im = t2_masked.mean(axis = -1).detach().cpu().numpy()
