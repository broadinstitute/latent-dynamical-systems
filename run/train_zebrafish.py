...
## TODO: validate these boolean arguments
LEARN_BASELINE_FACTORS = False
if args.learn_baseline_factors == "True":
    LEARN_BASELINE_FACTORS = True

INITIALIZE_MIXER = False
if str(args.initialize_mixer) == "True":
    INITIALIZE_MIXER = True

# MIXER_REG_TYPE = args.mixer_reg_type
...
# print('num genes being used: {}'.format(gex.shape[1]))
...
if INITIALIZE_MIXER and args.mix == 'nmf':
    nmf_model = mixer.initialize_mixer(gex)
    # save nmf_model as a pickle file with same prefix as the run
    with open(os.path.join(OUT_DIR, OUT_PREFIX + '_nmf_model.pkl'), 'wb') as f:
        pickle.dump(nmf_model, f)

    print(f"NMF model saved to {os.path.join(OUT_DIR, OUT_PREFIX + '_nmf_model.pkl')}") 
...
def evaluate_model(mixer,pde,T0,its, VALIDATION_TIMEPOINTS, VAL_TIME_OFFSETS, register_val = REGISTER_VAL):
    ...
    # wandb.log({'val/ge_reg_{}'.format(k): wandb.Histogram(v) for k,v in ge_loss_reg.items()}, step = its) ## skip histogram logging
    # wandb.log({'val/zero_reg_{}'.format(k): wandb.Histogram(v) for k,v in zero_loss_reg.items()}, step = its) ## skip histogram logging
...
for its in tqdm(range(MAX_ITS)):
    ...
    for sel_ind, sel in enumerate(CHOSEN_TIMEPOINTS):
        ...
        if sel_ind == 0:
            ...
            temp = initial ## TODO: double check steps
